...
.comment-area {
    ...
    box-sizing: border-box; /* Make sure padding is included in the element's total width/height */
}
...
.post-main {
    ...
    padding: 10px;
    margin-bottom: 10%; /* Leave 10% at the bottom for the comment area */
}
...
.comment {
    margin: 10px; /* Add margin to create a gap between the edge */
...
.comment-content {
    margin-top: 5px; /* Add space between username and comment content */
}
...
.comment-content p {
    margin: 0; /* Remove default margin for paragraphs within comment content */
}
...
$('.share-button').click(function () {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://localhost:8080/apiv1-internal/submit_post_api.php', // Replace with the correct URL
        data: formData,
...
for (let i = 0; i < data.length; i++) {
    ...
    // Create the comment area and add it to the post
    const commentArea = document.createElement('div');
    commentArea.className = 'comment-main';

    // Add "Hide/Show Comments" link at the top
    ...
    // Append the comment area to the post
    ...
    $.ajax({
        ...
        success: function (commentsData) {
            if (commentsData.status === 'success') {
                // Loop through the comments and append them to the 'comment-main' section
                $.each(commentsData.comments, function (index, comment) {
                    const commentElement = document.createElement('div');
                    commentElement.className = 'comment';
                    commentElement.innerHTML = `
                    <div class="pfp"></div> <!-- Placeholder for profile picture -->
                    <p class="username">${comment.username}</p>
                    <p class="comment-content">${comment.comment_content}</p>
                    `;

                    // Append the comment element to the 'comment-main' section
                    commentArea.appendChild(commentElement);
                });
            } else {
                // Handle the case when no comments are found
...
// Replace 'your_comments_api_endpoint.php' with the actual URL of your API endpoint
