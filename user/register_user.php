    ...
    if (!empty($username) && !empty($password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($conn->query($insertQuery) === TRUE) {
                // Read the content from tem.txt
                $profileContent = file_get_contents("tem.txt");
                
                // Replace placeholders if needed in tem.txt
                $profileContent = str_replace("{{username}}", $username, $profileContent);

                if (file_put_contents($profileFileName, $profileContent) !== false) {
